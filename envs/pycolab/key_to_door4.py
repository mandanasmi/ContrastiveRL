...
class KeySprite4(plab_things.Sprite):
  """Sprite for the key."""
...
class DoorSprite(plab_things.Sprite):
  ...
  def update(self, actions, board, layers, backdrop, things, the_plot):
    ...
    if pick_up and the_plot.get('has_key4'):
...
class PlayerSprite(common.PlayerSprite):
  ...
  def update(self, actions, board, layers, backdrop, things, the_plot):
    ...
    if (common.DOOR in self.impassable and the_plot.get('has_key')) or ( common.DOOR in self.impassable and the_plot.get('has_key2')) or ( common.DOOR in self.impassable and the_plot.get('has_key3')) or ( common.DOOR in self.impassable and the_plot.get('has_key4')):
...
class Game(game.AbstractGame):
  ...
  def __init__(self,
               ...
               final_reward=10.,
               crop=True,
               max_frames=common.DEFAULT_MAX_FRAMES_PER_PHASElong4keys):
  ...
  def _make_reward_phaseint3(self):
    rewardgrid = common.keep_n_characters_in_grid(
          REWARD_GRIDint3, common.BORDER2, 1)
    return ascii_art.ascii_art_to_game(
        art=rewardgrid,
        what_lies_beneath=' ',
        sprites={
            common.PLAYER: PlayerSprite,
            common.DOOR: ascii_art.Partial(DoorSpriteint3,
                                           pickup_reward=self._final_reward),
            common.BORDER2: KeySprite4,
            common.INDICATOR: ascii_art.Partial(objects.IndicatorObjectSprite,
                                                char_to_track=common.BORDER2,
                                                override_position=(0, 3)),
            common.TIMER: ascii_art.Partial(common.TimerSprite,
                                            self._max_frames['explore4'],
                                            track_chapter_reward=True),
        },
        update_schedule=[common.PLAYER, common.DOOR, common.BORDER2, common.INDICATOR, common.TIMER],
        z_order=[common.PLAYER, common.DOOR,common.BORDER2, common.INDICATOR, common.TIMER],
    )
