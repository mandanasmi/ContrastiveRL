...
class ReplayBuffer:
    def __init__(self, num_steps, batch_size, obs_shape, action_space, recurrent_hidden_state_size, capacity):
        

        # dtype = np.dtype([
        #     ('obs', torch.int, (nbytes,)),
        #     ('rewards', torch.int_, (1,)),
        #     ('actions', torch.int_, (1,)),
        #     ('returns', torch.bool_, (1,)),
        #     ('values', np.float_, (1,)),
        #     ('action_log_probs', np.float_, (1,)),
        #     ('masks', np.uint8, (nbytes,)),
        #     ('next_adjacency', np.uint8, (nbytes,)),
        #     ('next_mask', np.uint8, (nbytes,))
        # ])
        self._replay = np.zeros((capacity,))#, ddtypetype=)
        self.obs = torch.zeros(num_steps + 1, batch_size, *obs_shape)

        self.num_processes = batch_size
        self.recurrent_hidden_states = torch.zeros(num_steps + 1, batch_size, recurrent_hidden_state_size)

        self.rewards = torch.zeros(num_steps, batch_size, 1)
        self.rewardsORIG = torch.zeros(num_steps, batch_size, 1)
        self.value_preds = torch.zeros(num_steps + 1, batch_size, 1)
        self.returns = torch.zeros(num_steps + 1, batch_size, 1)
        self.action_log_probs = torch.zeros(num_steps, batch_size, 1)
        action_shape = 1
        self.actions = torch.zeros(num_steps, batch_size, action_shape)
        self.actions = self.actions.long()

        self.masks = torch.ones(num_steps + 1, batch_size, 1)

        # Masks that indicate whether it's a true terminal state
        # or time limit end state
        self.bad_masks = torch.ones(num_steps + 1, batch_size, 1)
